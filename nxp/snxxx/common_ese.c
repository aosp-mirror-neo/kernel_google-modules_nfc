...
static void cold_reset_guard_timer_callback(struct timer_list *t)
{
...
static long start_cold_reset_guard_timer(struct cold_reset *cold_reset)
{
	...
	timer_setup(&cold_reset->timer, cold_reset_guard_timer_callback, 0);
					 ...
					 unsigned long arg)
{
	...
	/* enable interrupt if not enabled in case when devnode not opened by HAL */
